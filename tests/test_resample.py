"""_to_mono / _resample_linear のテスト (Windows loopback の音声前処理).
...
from realtime_interpreter.backends.openai_realtime import _resample_linear, _to_mono
...
def test_to_mono_passthrough_1d() -> None:
    ...
    assert np.allclose(_to_mono(mono_in), mono_in)


def test_resample_same_rate_is_noop() -> None:
    x = np.linspace(-1, 1, 100, dtype=np.float32)
    out = _resample_linear(x, 24000, 24000)
...
def test_resample_downsample_length() -> None:
    ...
    out = _resample_linear(x, 44100, 24000)
    assert out.shape[0] == round(44100 * 24000 / 44100)
    assert out.shape[0] == 24000


def test_resample_empty() -> None:
    out = _resample_linear(np.zeros(0, dtype=np.float32), 44100, 24000)
...
def test_resample_dtype_is_float32() -> None:
    ...
    out = _resample_linear(x, 48000, 24000)
...
def test_resample_sine_roughly_preserved() -> None:
    ...
    out = _resample_linear(sine, 44100, 24000)
